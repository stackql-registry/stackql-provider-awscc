...
<tbody>
  ...
  <tr>
    ...
    <td><code>deliveries_list_only</code></td>
...
resources:
  - name: delivery
    props:
      ...
      - name: s3_enable_hive_compatible_path
        value: '{{ s3_enable_hive_compatible_path }}'`}</CodeBlock>
...
Use the following StackQL query and manifest file to update a <code>delivery</code> resource, using [__`stack-deploy`__](https://pypi.org/project/stack-deploy/).
