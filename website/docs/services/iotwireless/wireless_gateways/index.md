...
<tbody>
  ...
  <tr>
    ...
    <td><code>wireless_gateways</code></td>
  ...
  <tr>
    ...
    <td><code>wireless_gateways_list_only</code></td>
  ...
  - name: wireless_gateway
    props:
      ...
      - name: thing_name
        value: '{{ thing_name }}'
      - name: last_uplink_received_at
        value: '{{ last_uplink_received_at }}'`}</CodeBlock>
...
Use the following StackQL query and manifest file to update a <code>wireless_gateway</code> resource, using [__`stack-deploy`__](https://pypi.org/project/stack-deploy/).
