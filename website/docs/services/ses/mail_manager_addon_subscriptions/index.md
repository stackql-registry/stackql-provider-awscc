...
<tr><td><b>Id</b></td><td><CopyableCode code="awscc.ses.mail_manager_addon_subscriptions" /></td></tr>
</tbody>
</table>
...
WHERE region = 'us-east-1' AND Identifier = '<AddonSubscriptionId>';
```
</TabItem>
<TabItem value="list">
...
Use the following StackQL query and manifest file to update a <code>mail_manager_addon_subscription</code> resource, using [__`stack-deploy`__](https://pypi.org/project/stack-deploy/).
...
WHERE region = '{{ region }}'
AND Identifier = '<AddonSubscriptionId>';
```
...
WHERE Identifier = '<AddonSubscriptionId>'
