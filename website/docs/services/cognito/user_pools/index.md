...
resources:
  - name: user_pool
    props:
      ...
      - name: user_pool_tier
        value: '{{ user_pool_tier }}'`}</CodeBlock>
...
Use the following StackQL query and manifest file to update a <code>user_pool</code> resource, using [__`stack-deploy`__](https://pypi.org/project/stack-deploy/).
