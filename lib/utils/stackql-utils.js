...
function generateSelectQuery({
  ...
  const typeName = schema['x-type-name'];
...
export function generateStackqlViews(openApiSpec) {
          ...
          post: {
            ...
            responses: {
              '200': {
                content: {
                  "application/json": {
                    schema: { $ref: '#/components/x-cloud-control-schemas/ProgressEventResponse' }
                  }
                },
                description: "Success"
              }
  ...
  // filter out resources with no methods and no config (views)
  const excludedResources = [];
  for (const [resourceName, resourceDef] of Object.entries(views)) {
    const hasMethods = Object.keys(resourceDef.methods || {}).length > 0;
    const hasConfig = !!resourceDef.config;
    if (!hasMethods && !hasConfig) {
      excludedResources.push(resourceName);
      delete views[resourceName];
    }
  }

  if (excludedResources.length > 0) {
    console.log(`Excluded ${excludedResources.length} resource(s) with no methods or views: ${excludedResources.join(', ')}`);
  }
